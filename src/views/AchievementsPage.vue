<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-blue-50">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24">
      ...
      <div class="glass-card rounded-3xl p-6 mb-8">
        <div class="text-center mb-6">
          <div class="w-20 h-20 mx-auto mb-4 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="text-center p-4 bg-amber-50 rounded-xl">
            <div class="text-3xl font-bold text-amber-600 mb-2">{{ earnedAchievements.length }}</div>
            <div class="text-sm text-slate-600">已获得徽章</div>
          </div>
          <div class="text-center p-4 bg-blue-50 rounded-xl">
            <div class="text-3xl font-bold text-blue-600 mb-2">{{ allAchievements.length }}</div>
            <div class="text-sm text-slate-600">全部徽章</div>
          </div>
          <div class="text-center p-4 bg-green-50 rounded-xl">
            <div class="text-3xl font-bold text-green-600 mb-2">{{ allAchievements.length > 0 ? Math.round((earnedAchievements.length / allAchievements.length) * 100) : 0 }}%</div>
        ...
        <div class="space-y-4">
          ...
          >
            <div :class="[
              ...
              <component :is="achievement.icon" :class="['w-6 h-6', achievement.iconColor]" />
            </div>
            <div class="flex-1">
              <div class="font-medium text-slate-800">{{ achievement.name }}</div>
              <div class="text-sm text-slate-600">{{ achievement.description }}</div>
            </div>
            <div class="text-right">
              <div class="text-sm font-medium text-amber-600">{{ achievement.earnedDate }}</div>
              <div class="text-xs text-slate-500">+{{ achievement.points }}积分</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- 成就详情模态框 -->
    <div v-if="selectedAchievement" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="glass-card rounded-3xl p-6 w-full max-w-md">
        <div class="text-center mb-6">
          <div :class="[
            'w-20 h-20 mx-auto mb-4 rounded-full flex items-center justify-center',
            selectedAchievement.earned ? selectedAchievement.bgColor : 'bg-slate-300'
          ]">
            <component 
              :is="selectedAchievement.icon" 
              :class="[
                'w-10 h-10',
                selectedAchievement.earned ? selectedAchievement.iconColor : 'text-slate-500'
              ]" 
            />
          </div>
          <h3 class="text-xl font-bold text-slate-800 mb-2">{{ selectedAchievement.name }}</h3>
          <p class="text-slate-600 mb-4">{{ selectedAchievement.description }}</p>
          
          <div v-if="selectedAchievement.earned" class="space-y-2">
            <div class="text-green-600 font-medium">已获得</div>
            <div class="text-sm text-slate-500">获得时间: {{ selectedAchievement.earnedDate }}</div>
            <div class="text-sm text-slate-500">奖励积分: +{{ selectedAchievement.points }}</div>
          </div>
          <div v-else class="space-y-2">
            <div class="text-slate-500">未获得</div>
            <div class="text-sm text-slate-500">进度: {{ selectedAchievement.progress }}/{{ selectedAchievement.target }}</div>
...
const allAchievements = computed(() => {
  return achievementTemplates.map(template => {
    const userAchievement = userStore.achievements.find(a => a.id === template.id)
...
const filteredAchievements = computed(() => {
  ...
  return allAchievements.value.filter(achievement => achievement.category === selectedCategory.value)
})
