    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24">
      ...
      <div class="glass-card rounded-3xl p-6 mb-6">
        <div class="text-center mb-6">
          <div class="text-4xl font-bold text-blue-600 mb-2">{{ currentPoints }}</div>
          <div class="text-slate-600">当前积分余额</div>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="text-center">
            <div class="text-xl font-bold text-emerald-600 mb-1">{{ totalEarned }}</div>
            <div class="text-sm text-slate-500">累计获得</div>
          </div>
          <div class="text-center">
            <div class="text-xl font-bold text-rose-600 mb-1">{{ totalSpent }}</div>
            <div class="text-sm text-slate-500">累计消费</div>
          </div>
          <div class="text-center">
            <div class="text-xl font-bold text-violet-600 mb-1">{{ thisMonthEarned }}</div>
            <div class="text-sm text-slate-500">本月获得</div>
          </div>
          <div class="text-center">
            <div class="text-xl font-bold text-amber-600 mb-1">{{ currentLevel }}</div>
      ...
      <div class="glass-card rounded-2xl p-6 mb-6">
        ...
        <div class="flex items-center justify-between mb-3">
          <span class="text-sm font-medium text-slate-600">{{ currentLevel }}</span>
          <span class="text-sm font-medium text-slate-600">{{ nextLevel }}</span>
        </div>
...
const filteredRecords = computed(() => {
  ...
  if (selectedFilter.value === 'earn' || selectedFilter.value === 'spend') {
    return pointsRecords.filter(record => record.type === selectedFilter.value)
  }
  return pointsRecords.filter(record => record.category === selectedFilter.value)
})
