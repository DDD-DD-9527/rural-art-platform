<template>
  ...
  >
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24">
      ...
      <div
        v-if="activeNavTab === '推荐'"
        class="glass-card rounded-2xl p-6 mb-8"
      >
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          >
            <div class="flex items-center mb-2">
              <component
                :is="category.icon"
                class="w-6 h-6 mr-2 text-slate-600 group-hover:text-emerald-600 transition-colors"
              />
...
const fetchTopicCategories = async () => {
  try {
    ...
    const defaultTopics = [
      {
        name: "传统艺术",
        description:
          COURSE_CONFIG.CATEGORIES.slice(0, 3).join("、") + "等传统手工艺",
        count: 0,
        icon: PaletteIcon,
        color: "#FF6B6B",
      },
    ...
    const response = await fetch(
      `/api/topics/categories?limit=20&_t=${Date.now()}`,
      {
        method: "GET",
        headers: {
...
const mergePostsWithLocalState = (newPosts, existingPosts) => {
  console.log("mergePostsWithLocalState 输入参数:", {
    newPosts,
    existingPosts,
  });
  ...
  newPosts.forEach((post, index) => {
    if (post.author) {
      console.log(
        `帖子 ${index + 1} 作者 ${post.author._id} 关注状态: ${post.author.isFollowing}, 点赞状态: ${post.isLiked}`,
      );
    }
  });
...
const fetchPosts = async (page = 1, forceRefresh = false) => {
  try {
    ...
    if (activeNavTab.value === "推荐") {
      ...
      if (
        recommendedResponse &&
        recommendedResponse.posts &&
        recommendedResponse.posts.length > 0
      ) {
...
// 初始化数据
onMounted(async () => {
  await Promise.all([fetchTopicCategories(), fetchPosts(1, true)]);
