    ...
    <header class="glass-morphism border-b border-white/30 sticky top-0 z-40">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          ...
          <h1 class="text-lg font-bold text-slate-800">{{ currentViewTitle }}</h1>
          <div class="w-16"></div>
        </div>
      </div>
    </header>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24">
      ...
      <div class="glass-card rounded-2xl p-4 mb-6">
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="flex items-center space-x-2">
            <button @click="prevPeriod" class="p-2 hover:bg-slate-100 rounded-lg transition-colors">
              <ChevronLeftIcon class="w-5 h-5 text-slate-600" />
            </button>
            <div class="text-center min-w-[120px]">
              <div class="text-sm font-semibold text-slate-800">{{ currentPeriodText }}</div>
            </div>
            <button @click="nextPeriod" :disabled="isCurrentPeriod" class="p-2 hover:bg-slate-100 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div class="glass-card rounded-2xl p-6 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="text-center p-4 bg-blue-50 rounded-xl">
            <div class="text-2xl font-bold text-blue-600">{{ studyDaysCount }}</div>
            <div class="text-sm text-slate-500">学习天数</div>
          </div>
          <div class="text-center p-4 bg-emerald-50 rounded-xl">
            <div class="text-2xl font-bold text-emerald-600">{{ totalStudyMinutes }}</div>
            <div class="text-sm text-slate-500">总学习时长(分钟)</div>
          </div>
          <div class="text-center p-4 bg-violet-50 rounded-xl">
            <div class="text-2xl font-bold text-violet-600">{{ completedCoursesCount }}</div>
            <div class="text-sm text-slate-500">完成课程</div>
          </div>
          <div class="text-center p-4 bg-amber-50 rounded-xl">
            <div class="text-2xl font-bold text-amber-600">{{ totalEarnedPoints }}</div>
      ...
      <div class="glass-card rounded-2xl p-6 mb-6">
        ...
        <div class="grid grid-cols-7 gap-2">
          ...
          >
            ...
            <div v-if="date.hasStudy && date.isCurrentMonth" class="flex items-center mt-1">
              <div class="w-1.5 h-1.5 bg-emerald-500 rounded-full mr-1"></div>
              <span class="text-xs text-emerald-600">{{ date.studyMinutes }}分</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="glass-card rounded-2xl p-6">
        ...
        <div class="space-y-4">
          <div v-for="record in paginatedStudyRecords" :key="record.id" class="bg-white/50 rounded-xl p-4 border border-white/30">
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center space-x-3">
                ...
                <div>
                  <div class="font-semibold text-slate-800">{{ record.date }}</div>
                  <div class="text-sm text-slate-500">{{ record.dayOfWeek }}</div>
                </div>
              </div>
              <div class="text-right">
                <div class="text-lg font-bold text-emerald-600">{{ record.totalMinutes }}分钟</div>
                <div class="text-sm text-slate-500">{{ record.sessionsCount }}次学习</div>
              </div>
            </div>
            
            <div class="space-y-2">
              <div v-for="session in record.sessions" :key="session.id" class="flex items-center justify-between py-2 px-3 bg-slate-50 rounded-lg">
                <div class="flex items-center space-x-2">
                  <span class="text-sm font-medium text-slate-700">{{ session.courseName }}</span>
                  <span class="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-full">{{ session.category }}</span>
                </div>
        ...
        <div v-if="totalPages > 1" class="mt-6 pt-4 border-t border-slate-200">
          ...
          <div class="text-center mt-4 text-sm text-slate-500">
            第 {{ currentPage }} 页，共 {{ totalPages }} 页，总计 {{ studyRecords.length }} 条记录
          </div>
        </div>
      </div>

      <!-- 空状态 -->
      <div v-if="studyRecords.length === 0" class="glass-card rounded-2xl p-12 text-center mt-6">
        <CalendarIcon class="w-16 h-16 text-slate-300 mx-auto mb-4" />
        <h3 class="text-lg font-semibold text-slate-600 mb-2">{{ currentPeriodText }}暂无学习记录</h3>
...
const totalStudyMinutes = computed(() => {
  return studyRecords.value.reduce((total, record) => total + record.totalMinutes, 0)
})

const completedCoursesCount = computed(() => {
  return studyRecords.value.reduce((total, record) => total + record.sessionsCount, 0)
})

const totalEarnedPoints = computed(() => {
  return studyRecords.value.reduce((total, record) => {
    return total + record.sessions.reduce((sessionTotal, session) => sessionTotal + session.points, 0)
  }, 0)
})
...
const calendarDates = computed(() => {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    const dateKey = formatDateKey(date)
    const studyData = allStudyData[dateKey]
    const isCurrentMonth = viewMode.value === 'week' || date.getMonth() === currentDate.value.getMonth()
...
const totalPages = computed(() => Math.ceil(studyRecords.value.length / pageSize.value))
