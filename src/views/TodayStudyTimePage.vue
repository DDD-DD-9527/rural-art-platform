    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24">
      ...
      <div class="glass-card rounded-2xl p-4 mb-6">
        <div class="flex items-center justify-between">
          <button @click="prevDay" class="p-2 hover:bg-slate-100 rounded-lg transition-colors">
            <ChevronLeftIcon class="w-5 h-5 text-slate-600" />
          </button>
          <div class="text-center">
            <div class="text-lg font-semibold text-slate-800">{{ formatDate(selectedDate) }}</div>
            <div class="text-sm text-slate-500">{{ getDayOfWeek(selectedDate) }}</div>
          </div>
          <button @click="nextDay" :disabled="isToday" class="p-2 hover:bg-slate-100 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div class="glass-card rounded-2xl p-6 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div class="text-center p-4 bg-blue-50 rounded-xl">
            <div class="text-2xl font-bold text-blue-600">{{ studySessions.length }}</div>
          ...
          <div class="text-center p-4 bg-amber-50 rounded-xl">
            <div class="text-2xl font-bold text-amber-600">{{ earnedPoints }}</div>
      ...
      <div class="glass-card rounded-2xl p-6 mb-6">
        ...
        <div class="relative">
          <!-- 时间轴线 -->
          <div class="absolute left-6 top-0 bottom-0 w-0.5 bg-gradient-to-b from-emerald-200 to-blue-200"></div>
          
          <div class="space-y-6">
            <div v-for="(session, index) in paginatedSessions" :key="session.id" class="relative flex items-start space-x-4">
              <!-- 时间点 -->
              <div class="w-12 h-12 rounded-full bg-gradient-to-br from-emerald-400 to-blue-500 flex items-center justify-center flex-shrink-0 relative z-10">
                <component :is="session.icon" class="w-6 h-6 text-white" />
              </div>
              
              <!-- 学习内容 -->
              <div class="flex-1 bg-white/50 rounded-xl p-4 border border-white/30">
                <div class="flex items-center justify-between mb-2">
                  <div class="flex items-center space-x-2">
                    <span class="font-semibold text-slate-800">{{ session.courseName }}</span>
                    <span class="px-2 py-1 bg-emerald-100 text-emerald-700 text-xs rounded-full">{{ session.category }}</span>
                  </div>
                  <div class="text-sm text-slate-500">{{ session.startTime }} - {{ session.endTime }}</div>
                </div>
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center space-x-2">
                    <div class="w-2 h-2 rounded-full" :class="session.status === 'completed' ? 'bg-emerald-500' : 'bg-amber-500'"></div>
                    <span class="text-sm" :class="session.status === 'completed' ? 'text-emerald-600' : 'text-amber-600'">
      ...
      <div v-if="totalPages > 1" class="glass-card rounded-2xl p-4 mt-6">
        ...
        <div class="text-center mt-4 text-sm text-slate-500">
          第 {{ currentPage }} 页，共 {{ totalPages }} 页，总计 {{ studySessions.length }} 条记录
        </div>
      </div>

      <!-- 空状态 -->
      <div v-if="studySessions.length === 0" class="glass-card rounded-2xl p-12 text-center">
        <BookOpenIcon class="w-16 h-16 text-slate-300 mx-auto mb-4" />
        <h3 class="text-lg font-semibold text-slate-600 mb-2">今日暂无学习记录</h3>
...
const totalMinutes = computed(() => {
  return studySessions.value.reduce((total, session) => total + session.duration, 0)
})

const completedCourses = computed(() => {
  return studySessions.value.filter(session => session.status === 'completed').length
})

const earnedPoints = computed(() => {
  return studySessions.value.reduce((total, session) => total + session.points, 0)
})

const totalPages = computed(() => Math.ceil(studySessions.value.length / pageSize.value))
