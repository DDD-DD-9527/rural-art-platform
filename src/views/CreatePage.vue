    ...
    <main class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        ...
        >
          <div class="p-6">
            ...
            <h3 class="text-lg font-semibold mb-2 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-emerald-600 group-hover:to-blue-600 group-hover:bg-clip-text transition-all duration-300">
              {{ tool.title }}
            </h3>
      ...
      <div class="glass-effect rounded-3xl p-6 mb-8">
        <div class="flex items-center justify-between mb-6">
          ...
          <button @click="goToMyWorks" class="text-emerald-600 hover:text-emerald-700 font-medium transition-colors">
            查看全部
          </button>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          >
            ...
            <div class="absolute inset-0 flex flex-col justify-end p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <h4 class="text-white font-semibold text-sm mb-1">{{ creation.title }}</h4>
              <p class="text-white/80 text-xs mb-2">{{ creation.type }}</p>
              <div class="flex items-center justify-between">
                <span class="text-white/60 text-xs">{{ creation.createdAt }}</span>
        ...
        <div class="space-y-4">
          ...
          >
            <div class="relative flex-shrink-0">
              <img 
                :src="tutorial.thumbnail || '/placeholder.svg?height=80&width=120'"
                :alt="tutorial.title"
                class="w-30 h-20 object-cover rounded-xl"
              />
              <div class="absolute inset-0 flex items-center justify-center bg-black/20 rounded-xl group-hover:bg-black/10 transition-colors duration-200">
            ...
            <div class="flex-1">
              <h3 class="font-semibold text-slate-800 mb-1 group-hover:text-emerald-600 transition-colors duration-200">
                {{ tutorial.title }}
              </h3>
              <p class="text-slate-600 text-sm mb-2">{{ tutorial.description }}</p>
