<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-blue-50">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24">
      ...
      <div class="glass-card rounded-3xl p-6 mb-8">
        <div class="text-center">
          ...
          <h2 class="text-2xl font-bold text-slate-800 mb-2">共发表 {{ comments.length }} 条评论</h2>
      ...
      <div class="glass-card rounded-3xl p-6 mb-8">
        <div class="flex flex-col sm:flex-row gap-4">
          ...
          <select v-model="filterType" @change="handleFilterChange" class="px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
            <option value="all">全部类型</option>
            <option value="artwork">艺术作品</option>
            <option value="course">课程内容</option>
            <option value="post">社区动态</option>
          </select>
          <select v-model="sortBy" @change="handleSortChange" class="px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          >
              ...
              <div class="flex items-center space-x-3">
                ...
                <div>
                  <div class="flex items-center space-x-2 text-sm text-slate-500">
                    ...
                    <span :class="getTypeColor(comment.targetType)">{{ getTypeText(comment.targetType) }}</span>
                  </div>
                </div>
              </div>
            ...
            <div class="mb-4 p-4 bg-slate-50 rounded-xl">
              <div class="flex items-center space-x-3 mb-2">
                <img 
                  :src="comment.target.author?.avatar || '/placeholder-user.jpg'" 
                  :alt="comment.target.author?.nickname || '用户'"
                  class="w-8 h-8 rounded-full object-cover border border-white shadow-sm"
                />
                <div>
                  <h4 class="font-medium text-slate-800 text-sm">{{ comment.target.author?.nickname || comment.target.author?.username || '匿名用户' }}</h4>
                </div>
              </div>
              <h5 class="font-medium text-slate-700 mb-2">{{ comment.target.title }}</h5>
              <p class="text-sm text-slate-600 line-clamp-2">{{ comment.target.content }}</p>
            </div>
            ...
            <!-- 回复列表 -->
            <div v-if="comment.recentReplies && comment.recentReplies.length > 0" class="mt-4 pl-6">
              <div class="space-y-3">
                <div 
                ...
                >
                  ...
                  <div class="flex-1">
                    <div class="flex items-center space-x-2 mb-1">
                      <span class="text-sm font-medium text-slate-700">{{ reply.author.name }}</span>
    ...
    <!-- 图片预览模态框 -->
    <div v-if="previewImage" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
...
const loadMyComments = async () => {
  try {
    ...
    if (response.success) {
      ...
      hasMore.value = currentPage.value < (response.data.pagination?.pages || 1)
    }
  } catch (err) {
    error.value = '加载评论失败'
