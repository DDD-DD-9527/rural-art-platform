    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24">
      ...
      <div class="glass-card rounded-3xl p-6 mb-8">
        <div class="text-center">
          ...
          <h2 class="text-2xl font-bold text-slate-800 mb-2">共点赞 {{ likedItems.length }} 个内容</h2>
      ...
      <div class="glass-card rounded-3xl p-6 mb-8">
        <div class="flex flex-col sm:flex-row gap-4">
          ...
          <select v-model="filterType" class="px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500">
            <option value="all">全部类型</option>
            <option value="Post">社区动态</option>
            <option value="Comment">评论</option>
            <option value="Course">课程内容</option>
          </select>
          <select v-model="sortBy" class="px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500">
          ...
          >
            ...
            <div class="flex items-start justify-between mb-4">
              <div class="flex items-center space-x-3">
                ...
                />
                <div>
                  <h3 class="font-medium text-slate-800">{{ item.author.name }}</h3>
                  <div class="flex items-center space-x-2 text-sm text-slate-500">
                    <span>{{ item.publishDate }}</span>
                    <span class="w-1 h-1 bg-slate-300 rounded-full"></span>
                    <span :class="getTypeColor(item.type)">{{ getTypeText(item.type) }}</span>
                  </div>
                </div>
              </div>
              <div class="flex items-center space-x-2">
                <span class="text-sm text-slate-400">{{ item.likeDate }} 点赞</span>
                <button 
            ...
            <!-- 内容主体 -->
            <div class="mb-4">
              <h4 class="text-lg font-semibold text-slate-800 mb-2">{{ item.title }}</h4>
...
const loadLikedItems = async () => {
  try {
    ...
    const formattedLikes = response.data.likes.map(like => ({
      id: like.id,
      type: like.type === 'Post' ? 'post' : like.type === 'Comment' ? 'comment' : like.type === 'Course' ? 'course' : like.type.toLowerCase(),
