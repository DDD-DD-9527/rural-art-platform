  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          ...
          <button @click="showSortMenu = !showSortMenu" class="relative">
            <SlidersHorizontalIcon class="w-5 h-5 text-slate-600 hover:text-slate-900 transition-colors" />
            <!-- 排序菜单 -->
            <div v-if="showSortMenu" class="absolute right-0 top-8 w-48 glass-effect rounded-2xl shadow-lg border border-white/20 py-2 z-50">
              <button 
                ...
                :class="[
                  ...
                  currentSort === option.value ? 'text-emerald-600 bg-emerald-50' : 'text-slate-700 hover:bg-slate-50'
                ]"
              >
                {{ option.label }}
              </button>
    ...
    <main class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24">
      ...
      <div class="glass-effect rounded-3xl p-6 mb-8">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="text-center">
            <div class="text-2xl font-bold text-emerald-600 mb-1">{{ statistics.total }}</div>
            <div class="text-sm text-slate-600">总作品数</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-blue-600 mb-1">{{ statistics.thisMonth }}</div>
            <div class="text-sm text-slate-600">本月创作</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-purple-600 mb-1">{{ statistics.totalLikes }}</div>
            <div class="text-sm text-slate-600">获得点赞</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-orange-600 mb-1">{{ statistics.totalViews }}</div>
        ...
        >
          <div class="aspect-square relative">
            ...
            <div class="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <h3 class="text-white font-semibold text-sm mb-1">{{ work.title }}</h3>
      ...
      <div v-if="filteredWorks.length === 0" class="text-center py-16">
        <div class="w-24 h-24 bg-gradient-to-br from-slate-200 to-slate-300 rounded-full flex items-center justify-center mx-auto mb-4">
    ...
    <!-- 作品详情模态框 -->
    <div v-if="selectedWork" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click="closeWorkDetail">
      <div class="glass-effect rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" @click.stop>
        <div class="p-6">
          <div class="flex items-center justify-between mb-6">
            <h2 class="text-2xl font-bold text-slate-800">{{ selectedWork.title }}</h2>
            <button @click="closeWorkDetail" class="text-slate-500 hover:text-slate-700">
              <XIcon class="w-6 h-6" />
            </button>
          </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              <img 
                :src="selectedWork.image || '/placeholder.svg?height=400&width=400'"
            ...
            <div class="space-y-4">
              <div>
                ...
                <div class="space-y-2 text-sm">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span class="text-slate-800">{{ selectedWork.createdAt }}</span>
                  </div>
                  <div class="flex justify-between">
              ...
              <div v-if="selectedWork.description">
                ...
                <p class="text-sm text-slate-600">{{ selectedWork.description }}</p>
              </div>
...
const allWorks = reactive([
  {
    ...
    likes: 24,
    views: 156,
    description: '传统剪纸艺术与现代AI技术的完美结合，展现了春天花朵的美丽与生机。'
  },
  {
    id: 2,
    title: '乡村风景',
