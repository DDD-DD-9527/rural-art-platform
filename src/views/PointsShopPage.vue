    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24">
      ...
      <div class="glass-card rounded-3xl p-6 mb-8">
        <div class="text-center">
          ...
          <div class="text-3xl font-bold text-violet-600 mb-2">{{ userPoints }}</div>
          <div class="text-slate-500 mb-4">当前积分</div>
          <div class="flex justify-center space-x-4">
            <div class="text-center">
              <div class="text-lg font-semibold text-slate-800">{{ monthlyEarned }}</div>
              <div class="text-sm text-slate-500">本月获得</div>
            </div>
            <div class="text-center">
              <div class="text-lg font-semibold text-slate-800">{{ totalSpent }}</div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          <div class="aspect-square bg-gradient-to-br from-slate-200 to-slate-300 rounded-xl mb-4 flex items-center justify-center overflow-hidden">
            <img v-if="item.image" :src="item.image" :alt="item.name" class="w-full h-full object-cover" />
            <component v-else :is="item.icon" class="w-12 h-12 text-slate-500" />
          </div>
          ...
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center space-x-1">
              ...
              <span class="text-lg font-bold text-violet-600">{{ item.points }}</span>
              <span class="text-sm text-slate-500">积分</span>
            </div>
            <div v-if="item.originalPrice" class="text-sm text-slate-400 line-through">
              ¥{{ item.originalPrice }}
            </div>
          </div>
        ...
        <div class="space-y-4">
          ...
          >
            <div class="flex items-center space-x-4">
              <div class="w-12 h-12 bg-gradient-to-br from-violet-400 to-purple-500 rounded-lg flex items-center justify-center">
                <ShoppingBagIcon class="w-6 h-6 text-white" />
              </div>
              <div>
                <div class="font-medium text-slate-800">{{ record.itemName }}</div>
                <div class="text-sm text-slate-500">{{ record.date }}</div>
              </div>
            </div>
            <div class="text-right">
              <div class="text-violet-600 font-semibold">-{{ record.points }}积分</div>
              <div :class="[
                'text-sm px-2 py-1 rounded-full',
                record.status === '已发货' ? 'bg-green-100 text-green-600' :
