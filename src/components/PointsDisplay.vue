  ...
  <div class="points-display">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="text-center">
          <div class="relative inline-block">
            <div class="w-24 h-24 mx-auto mb-4 rounded-full bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center shadow-xl">
              <StarIcon class="w-12 h-12 text-white" />
            </div>
            <div class="absolute -top-2 -right-2 bg-emerald-500 text-white rounded-full px-2 py-1 text-xs font-bold">
              +{{ recentPoints }}
            </div>
          </div>
          <div class="text-4xl font-bold text-yellow-600 mb-2">{{ formatNumber(totalPoints) }}</div>
          <div class="text-lg text-slate-600">总积分</div>
          <div class="text-sm text-emerald-600 mt-1">今日 +{{ todayPoints }}</div>
        </div>
        
        <!-- 当前等级 -->
        <div class="text-center">
          <div class="w-24 h-24 mx-auto mb-4 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center shadow-xl relative">
            <TrophyIcon class="w-12 h-12 text-white" />
            <div class="absolute -bottom-2 bg-white text-purple-600 rounded-full px-3 py-1 text-sm font-bold shadow-lg">
              Lv.{{ currentLevel }}
            </div>
          </div>
          <div class="text-2xl font-bold text-purple-600 mb-2">{{ levelName }}</div>
          ...
          <div class="mt-4">
            ...
            <div class="text-xs text-slate-500 mt-1">{{ levelProgress }}% 完成</div>
          </div>
        </div>
        
        <!-- 学习天数 -->
        <div class="text-center">
          <div class="w-24 h-24 mx-auto mb-4 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-xl">
            <CalendarIcon class="w-12 h-12 text-white" />
          </div>
          <div class="text-4xl font-bold text-emerald-600 mb-2">{{ studyDays }}</div>
          <div class="text-lg text-slate-600">学习天数</div>
          <div class="text-sm text-emerald-600 mt-1">连续 {{ streakDays }} 天</div>
        </div>
      </div>
    </div>
    ...
    <div class="points-sources glass-card rounded-3xl p-8 mb-8 shadow-xl">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <div :class="['w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center', source.bgClass]">
            <component :is="source.icon" :class="['w-8 h-8', source.iconClass]" />
          </div>
          <div class="text-lg font-bold text-slate-800 mb-2">{{ source.title }}</div>
          <div class="text-2xl font-bold mb-2" :class="source.pointsClass">+{{ source.points }}</div>
      ...
      <div class="space-y-4 max-h-96 overflow-y-auto">
        ...
        >
          <div class="flex items-center space-x-4">
            <div :class="['w-12 h-12 rounded-full flex items-center justify-center', getRecordIconBg(record.type)]">
              <component :is="getRecordIcon(record.type)" :class="['w-6 h-6', getRecordIconColor(record.type)]" />
            </div>
            <div>
              <div class="font-semibold text-slate-800">{{ record.reason }}</div>
              <div class="text-sm text-slate-600">{{ formatDate(record.createdAt) }}</div>
              <div v-if="record.courseTitle" class="text-xs text-slate-500">{{ record.courseTitle }}</div>
            </div>
          </div>
          <div class="text-right">
            <div :class="['text-xl font-bold', record.points > 0 ? 'text-emerald-600' : 'text-red-500']">
...
const filteredHistory = computed(() => {
  ...
  return pointsHistory.value.filter(record => 
