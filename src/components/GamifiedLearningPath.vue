  ...
  <div class="gamified-learning-path">
    ...
    <div class="path-header glass-card rounded-3xl p-8 mb-8 shadow-xl">
      <div class="flex items-center justify-between">
        <div class="flex-1">
          <h2 class="text-3xl font-bold mb-3 text-slate-800">
            {{ pathData.title }}
          </h2>
          ...
          <div class="flex items-center space-x-6 text-lg">
            <div class="flex items-center space-x-2">
              ...
              <span class="text-slate-700">
                <strong class="text-emerald-600">{{ completedLessons }}</strong>
                / {{ totalLessons }} 课程完成
              </span>
            </div>
            <div class="flex items-center space-x-2">
              ...
              <span class="text-slate-700">
                <strong class="text-yellow-600">{{ totalPoints }}</strong>
                积分获得
              </span>
            </div>
            <div class="flex items-center space-x-2">
    ...
    <!-- 学习路径可视化 -->
    <div
      class="path-visualization gradient-primary rounded-3xl p-8 text-white shadow-2xl"
    >
      <div class="relative overflow-x-auto">
        ...
        <!-- 课程节点 -->
        <div
          class="relative z-10 flex justify-between items-start min-w-max px-12 py-8"
        >
          <div
          ...
          >
            ...
            <div class="text-center min-w-[160px]">
              ...
              <div class="text-sm opacity-90 mb-3">
                <div
                  class="bg-white/20 px-3 py-1 rounded-full inline-block mb-2 font-medium"
                >
                  {{ lesson.type }}
                </div>
              ...
              <div
                v-if="lesson.status === 'locked'"
                class="text-sm text-white/70 mb-3"
              >
              ...
              <!-- 课程进度条 -->
              <div
                v-if="lesson.progress > 0 && lesson.status !== 'completed'"
                class="mt-3"
              >
          ...
          <div class="flex flex-col items-center min-w-0 flex-shrink-0 mx-4">
            <div
              class="relative w-28 h-28 rounded-full bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center mb-4 shadow-xl shadow-yellow-400/50"
            >
              <TrophyIcon class="w-14 h-14 text-white" />
              <div
                v-if="isPathCompleted"
                class="absolute -inset-3 rounded-full bg-yellow-400/30 animate-pulse"
              ></div>
            </div>
            <div class="text-center min-w-[140px]">
              <h4 class="font-bold text-xl mb-2">
                {{ pathData.certificate.title }}
              </h4>
              <div class="text-sm opacity-90 mb-3">
                <div
                  class="bg-white/20 px-3 py-1 rounded-full inline-block mb-2"
                >
                  证书奖励
                </div>
                <div class="text-yellow-300 font-bold">
                  +{{ pathData.certificate.points }} 积分
                </div>
              </div>
              <button
                v-if="isPathCompleted"
...
const isPathCompleted = computed(
  () => completedLessons.value === totalLessons.value,
);
