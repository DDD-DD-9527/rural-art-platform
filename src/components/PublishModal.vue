<template>
  ...
  >
    <div
      class="glass-effect rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl"
    >
      <!-- Header  -->
      <div
          ...
          <div class="w-12 h-12 rounded-full overflow-hidden">
            ...
            >
              <span class="text-white font-semibold">{{
                (userStore.user.name || userStore.user.username || "我").charAt(
                  0,
                )
              }}</span>
            </div>
          ...
          <div class="grid grid-cols-3 gap-3">
            ...
            >
              <div class="text-center">
                <PlusIcon
                  class="w-8 h-8 text-slate-400 group-hover:text-emerald-500 mx-auto mb-1"
                />
                <span
                  class="text-xs text-slate-500 group-hover:text-emerald-600"
                  >添加图片</span
                >
              </div>
            </button>
      ...
      <!-- Footer  -->
      <div
        class="flex items-center justify-between p-6 border-t border-slate-200/50"
      >
        <div class="flex items-center space-x-4 text-sm text-slate-500">
          <button
            @click="showLocationInput = !showLocationInput"
            class="flex items-center hover:text-slate-700 transition-colors"
          >
...
const canPublish = computed(() => {
  return (
    content.value && content.value.trim().length > 0 && !isPublishing.value
  );
});
...
const addCustomTag = () => {
  if (
    ...
    !selectedTags.includes(customTag.value.trim())
  ) {
