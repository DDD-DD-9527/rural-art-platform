...
export const useUserStore = defineStore("user", () => {
  ...
  const updateSkillProgress = async (skillType, skillName, progress) => {
    if (isAuthenticated.value) {
      // 调用API更新后端数据
      const success = await updateSkillProgressAPI(
        skillType,
        skillName,
        progress,
      );
    ...
    if (skillProgress[skillType]) {
      const skill = skillProgress[skillType].skills.find(
        (s) => s.name === skillName,
      );
      if (skill) {
      ...
      // 重新计算整体进度
      const totalProgress = skillProgress[skillType].skills.reduce(
        (sum, skill) => sum + skill.progress,
        0,
      );
      skillProgress[skillType].progress = Math.round(
        totalProgress / skillProgress[skillType].skills.length,
      );
    }
    return true;
  };
  ...
  const setGuestMode = () => {
    ...
    skillProgress.artSkills.skills = [{ name: "剪纸入门", progress: 30 }];
    skillProgress.digitalSkills.progress = 15;
    skillProgress.digitalSkills.skills = [{ name: "手机摄影", progress: 20 }];
  ...
  const updateLearningStats = async (stats) => {
    try {
      const data = await api.put("/users/learning-stats", stats);
      if (data?.success && data?.data?.learningStats) {
  ...
  const updateSkillProgressAPI = async (skillType, skillName, progress) => {
    try {
      const data = await api.put("/users/skill-progress", {
        skillType,
        skillName,
        progress,
      });
      if (data?.success && data?.data?.skillProgress) {
  ...
  const addAchievementAPI = async (achievement) => {
    try {
      const data = await api.post("/users/achievements", achievement);
