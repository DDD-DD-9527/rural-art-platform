...
export const useSocialStore = defineStore('social', () => {
  ...
  const initializePostsState = (posts) => {
    ...
    posts.forEach((post, index) => {
      if (post && (post._id || post.id)) {
        ...
        if (post.hasOwnProperty('isLiked')) {
          likeStates.value[postId] = Boolean(post.isLiked)
          if (oldLikeState !== Boolean(post.isLiked)) {
            console.log(`🔄 帖子 ${postId} 点赞状态更新: ${oldLikeState} -> ${Boolean(post.isLiked)}`)
          }
        } else if (!likeStates.value.hasOwnProperty(postId)) {
        ...
        if (oldLikeCount !== likeCount) {
          console.log(`🔢 帖子 ${postId} 点赞数更新: ${oldLikeCount} -> ${likeCount}`)
        }
        ...
        if (post.author && (post.author._id || post.author.id)) {
          ...
          if (post.author.hasOwnProperty('isFollowing')) {
            followStates.value[authorId] = Boolean(post.author.isFollowing)
            if (oldFollowState !== Boolean(post.author.isFollowing)) {
              console.log(`🔄 作者 ${authorId} 关注状态更新: ${oldFollowState} -> ${Boolean(post.author.isFollowing)}`)
            }
          } else if (!followStates.value.hasOwnProperty(authorId)) {
            // 只有在服务器没有返回状态且本地也没有缓存时才设为false
  ...
  const togglePostLike = async (postId) => {
    ...
    likeCounts.value[postIdStr] = Math.max(0, newLikeCount)
      ...
      const originalLikeCount = originalLikeState ? likeCounts.value[postIdStr] - 1 : likeCounts.value[postIdStr] + 1
  ...
  const clearAllStates = () => {
    Object.keys(likeStates.value).forEach(key => delete likeStates.value[key])
    Object.keys(followStates.value).forEach(key => delete followStates.value[key])
    Object.keys(likeCounts.value).forEach(key => delete likeCounts.value[key])
  ...
  const followUser = async (userId) => {
    ...
    try {
      ...
      console.error('关注操作失败:', error)
      const message = error.message?.includes('已经关注') ? '您已经关注了该用户' : '关注失败，请稍后重试'
      ElMessage.error(message)
      return { success: false, message }
    } finally {
      loading.value = false
  ...
  const unfollowUser = async (userId) => {
    ...
    try {
      ...
      const message = error.message?.includes('未关注') ? '您未关注该用户' : '取消关注失败，请稍后重试'
      ElMessage.error(message)
      return { success: false, message }
    } finally {
      loading.value = false
