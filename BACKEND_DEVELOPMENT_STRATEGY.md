...
您希望在本地开发新版本后端，同时保持当前生产环境的稳定运行，确保测试通过后再更新部署。
...
```bash
cd backend
git checkout -b feature/new-backend-version
```
...
MONGODB_URI=mongodb://localhost:27017/rural-art-platform-dev
```
...
```json
{
  "scripts": {
    ...
    "dev:new": "NODE_ENV=development PORT=3001 nodemon server.js"
  }
}
```
...
#### 测试流程
- **开发版本**：`http://localhost:5173/api/*` → `localhost:3001`（新后端）
- **生产版本**：修改 `VITE_PROXY_TARGET=http://localhost:3000` 后重启前端
